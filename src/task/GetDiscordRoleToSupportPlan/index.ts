...
export class GetDiscordRoleToSupportPlanTask extends Task {
    ...
    constructor(bot: DiscordBotClient, coolTime: number = 1440, errorHandler: Function = null) {
        super(errorHandler);
    ...
    public async execute(): Promise<void> {
        ...
        try {
            ...
            const planRoleMap = config.settings.roleIdForPlan;
            for (const planId in planRoleMap) {
                ...
                if (roles.has(roleId)) {
                    ...
                    this.logger.debug(`found role for plan. userId: ${userInfo.userId}, planId: ${planId}`);
                    return;
                }
            }
